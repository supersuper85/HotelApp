...
using MongoDB.Bson;
using MongoAuditApp.BLL.Validations;
...
namespace MongoAuditApp.BLL.Implementations
{
    public class MongoAuditService : IMongoAuditService
    {
        ...
        public async Task<MongoAuditDto> Delete(string id)
        {
            var audit = await _auditRepository.SingleOrDefaultAsync(x => x.Id == ObjectId.Parse(id));

            var auditValidator = new MongoAuditDatabaseValidator();
            auditValidator.CheckAuditDeleteModel(audit);

            var response = await _auditRepository.DeleteAsync(audit);
            return response ? _mapper.Map<MongoAuditDto>(audit) : null;
